...
We let $\mathcal{R}_r$ denote the set $\{PV,PP_1,\ldots,PP_{\kappa(r-1)} \} \subset \mathcal{P}_r$ to denote the set of non-ancillary provers. Furthermore, we order the provers in $\mathcal{P}_r$ for notational convenience: $PV < PP_1 < \cdots < PP_{\kappa(r-1)} < PA$. 
...
\begin{mdframed}
    ...
    Choose a random prover $P^*$ from the set of non-ancillary provers $\mathcal{R}_r$, and let player B denote prover $P^*$. \tnote{should we write we are telling them who they are, and who the other is?}\\
%    Let player A denote the ancillary prover corresponding to $P$ (see the Notation subsection above). \\
    \\
    Perform one of the tests uniformly at random. Each test should be interpreted as a three-phase procedure, as described in Figure~\ref{fig:check_structure}. \hnote{This is already stated in the paragraph above the box.}\tnote{Yeah but I'm strongly in favor of redundancy, especially with boxes. Someone might look up just the box, and not realize stuff is defined elsewhere. Since this is the ``template box'' I think it doesn't hurt. But I'm ok removing if you find it too ugly} The \textsc{ClockMeasurement} phase involves a measurement on the clock register $\reg{C}_p$ of dimension $2(N+1)(\kappa(r-1)+1)$, with computational basis labeled by 
    \[
    (d,t,P)\in \{0,1\}\times \{0,\ldots,N\} \times \mathcal{R}_r .
    \]
    
	\begin{enumerate}
 \item \emph{(EPR test)} Measure register $\reg{C}_{p}$ in the computational basis. Accept if the outcome $(t,P)\neq (0,P^*)$ (we do not care what $d$ is\tnote{who is ``we''??? Please don't write this kind of stuff, then I need to fix the language and add notes all over the place}). Otherwise, execute the $2N$-qubit EPR test with A and B, where A's questions are augmented by $P^*$ (i.e. which prover that player B corresponds to). \\
...
\]\tnote{earlier you wrote $\sP_P$ ``includes'' $\sB_P$ and $\sB'_P$, so it might seem like some registers are missing above}
...
\end{align}
\tnote{I think the second line should only have $CX_t$, because when $d=1$ this means all the $CZ_t$ have been applied already}
...
\begin{enumerate}
	...
	\item (\textbf{Soundness}) Any strategy $\mathcal{S}$ that passes the Pauli Check with probability at least $1 - \eps$ is $\delta$-isometric to an honest Pauli Check strategy for some $\delta = \poly(N,\eps)$.\tnote{seriously we need a better notation for it. How about, $\delta \in \poly[N,\eps]$, where we'd define this notation to mean, any function of $N$ and $\eps$ such that for fixed $\eps$, the function is bounded by some $\poly_\eps(N)$, and for fixed $N$, it is bounded by a $\poly_N(\eps)$ that moreover goes to $0$ as $\eps\to 0$ (i.e. no constant term). }
\end{enumerate}
\end{lemma}
