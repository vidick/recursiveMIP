...
\begin{mdframed}
    ...
    Choose a random prover $P^*$ from the set of non-ancilla provers $\mathcal{R}_r$, and let player B denote prover $P^*$. \tnote{should we write we are telling them who they are, and who the other is?} \hnote{It is important that player B not know we're performing the Pauli check, so we shouldn't be telling player B any additional information beyond the EPR/Pauli questions.} \\
    \\
	...
	\begin{enumerate}
 \item \emph{(EPR test)} Measure register $\reg{C}_{p}$ in the computational basis. Accept if the outcome $(t,P)\neq (0,P^*)$. Otherwise, execute the $2N$-qubit EPR test with A and B, where A's questions are augmented by $P^*$ (i.e. which prover that player B corresponds to). \\
...
\end{align}
\tnote{I think the second line should only have $CX_t$, because when $d=1$ this means all the $CZ_t$ have been applied already} \hnote{In what you wrote before, the operator applied was $X(d \cdot a_{\leq t}) Z (b_{\leq t})$ which seems like the $X$'s and $Z$'s are applied only up to the $t$'th qubit, together. Is this what you meant?}
