...
Thus we have
	...
	\]
	Notice that the operator $\E_t  \left ( Q\ketbra{-_t}{-_t}Q^\dagger  \right)$ is exactly the same as the propagation term of the Feynman-Kitaev Hamiltonian that checks the propagation of the computation $g_1 g_2 \cdots g_{\tau(N)}$. This implies that $\rho_0$ is $\poly(N,\eps)$-close to a history state of the computation $g_1g_2\cdots g_{\tau(N)}$.\tnote{should the last implication refer to a theorem?}
