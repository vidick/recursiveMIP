...
\end{align*}
Furthermore, for convenience we define $\exp^0_a(b) = b$.% and $\exp^0_a(-b) = 1/b$ when $b \geq 0$.
...
A verifier $V = (C_Q,C_A)$ implicitly define $3$-turn quantum interactive protocol for some number of provers. We let $\Protocol_V$ denote this protocol.
...
\end{enumerate}
There are a few properties that are inherent to a VTM $\cV$: First, there is a function $r_{\cV,n}$ that depends on the parameter $n$. On input $(1^n,w,\lambda)$, the prover question and answer tapes will be divided into $r_{\cV,n}$ components; $r_{\cV,n}$ denotes the number of provers the VTM write messages for. \hnote{Need to update}
...
Simulate $\cV$ on input $(1^n,w,\lambda)$ for $p(n)$ steps using TMSIM, where $\lambda$ is the contents of $\sV_{in}$. Let $s$ denote the contents of the measurement specification tape of $\cV$.   Execute CKTSIM on input $s$ and the registers $\sV_{work}, \sC$.
	...
	The circuit $C_A(n,\cV)$:
	  \begin{enumerate}    
        ...
        \medskip
       	We use $C_Q(n,\cV,\lambda)$ to denote the circuit $C_Q(n,\cV)$ where $\lambda$ is the contents of register $\sV_{in}$.
\end{mdframed}
\begin{figure}[H]
...
\label{lem:succinct_vtm_circuits}
	There exists a GTM $G_{C_Q}$ (resp. $G_{C_A}$) such that on input $(n,t,\cV)$ where $n,t$ are integers written in binary such that $1 \leq t \leq |C_Q(n,\cV)|$ (resp. $1 \leq t \leq |C_A(n,\cV)|$), computes the $t$-th gate of $C_Q(n,\cV)$ (resp. $C_A(n,\cV)$) in time $\poly \log_{|\cV|} (n)$.
\end{lemma}
\begin{proof}
...
\begin{enumerate}
	...
	\item The $\sO$ register is set to $1$ if and only if the $\sK_1$ register is all zero. \tnote{is this the only reason for $\sO$? Given later you consider measuring complete registers, $\sK_2\sK_2'$, why not do the same for $\sK_1$, is $\sO$ used for another reason?}\hnote{The $\sK_2 \sK_2'$ registers are constant sized, so we can (and have to) ask the provers to measure all of them. However, the $\sK_1$ register is $\poly(n)$ sized, but we can only ask $PV$ to measure a constant number of Pauli measurements.}
\end{enumerate}
