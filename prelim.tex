...
\label{lem:closeness_to_groundspace}
	Let $\Hilb_{\sA},\Hilb_{\sR}$ be Hilbert spaces. Let $H$ be a positive semidefinite matrix acting on $\Hilb_{\sA}$ with minimum eigenvalue $0$ and second smallest eigenvalue $\Delta > 0$. If $\ket{\psi}$ is a state on $\Hilb_{\sA} \otimes \Hilb_{\sR}$ such that $\bra{\psi} H_{\sA} \otimes \Id_{\sR} \ket{\psi} \leq \eps$, then there exists a state $\ket{\theta}$ in $\Hilb_{\sA} \otimes \Hilb_{\sR}$ such that $H \ket{\theta} = 0$ and
	\[
