...
A verifier $V = (C_Q,C_A)$ will\tnote{will ---  when? There are 25 wills in this draft. Let's cut it to 1/10-th of this --- rounded down.} implicitly define $3$-turn quantum interactive protocol for some number of provers. We let $\Protocol_V$ denote this protocol.
...
\paragraph{From VTMs to verifiers} We now specify how a VTM $\cV$ specifies a family of circuits $\{ (C_Q^{(n)},C_A^{(n)}) \}$ for the verifier. Fix the parameter $n$. The circuits will act on three registers $\sC, \sV, \sM$ of $3p(n)$ qubits each\tnote{let's not use the notion of a register being ``$3p(n)$ qubits wide''. It's ugly. And a register has no depth, so width is the only parameter, it doesn't need a name}. The $\sV$ register is divided into two subregisters $\sV_{in}$ and $\sV_{work}$ (each are $3p(n)/2$ qubits wide). 
...
Simulate $\cV$ on input $(1^n,w,\lambda)$ for $p(n)$ steps using TMSIM, where $\lambda$ is the contents of $\sV_{in}$. Let $s$ denote the contents of the measurement specification tape of $V_R$\tnote{is this $\sV$?}.   Execute CKTSIM on input $s$ and the $\sV_{work}, \sC$ registers.\tnote{Refer to ``the register $\sV$'', not ``the $\sV$ register''. Same for ``the $V$ verifier'', should be ``the verifier $V$'', etc.}%, writing the classical output $s$ into $\sV_{work}$.
%registers $\sV_{m}$ (circuit description) and $\sC$, writing the classical output in register $\sV_{mo}$.
	...
	The circuit $C_A^{(n)}$:
	  \begin{enumerate}    
        ...
        \medskip
       	We will use $C_Q^{(n)}(\lambda)$ to denote the circuit $C_Q^{(n)}$ where $\lambda$ is initialized in the register $\sV_{in}$\tnote{above you write something very similar as ``$\lambda$ is the contents of'', instead of ``$\lambda$ is initialized in''. Are they the same? I prefer the former}.
\end{mdframed}
\begin{figure}[H]
...
\label{lem:succinct_vtm_circuits}
	There exists a GTM $G_{C_Q}$ (resp. $G_{C_A}$) such that on input $(n,t,\cV)$ where $n,t$ are integers written in binary such that $1 \leq t \leq |C_Q^{(n)}|$ (resp. $1 \leq t \leq |C_A^{(n)}|$), computes the $t$-th gate of $C_Q^{(n)}$ (resp. $C_A^{(n)}$) that corresponds to $\cV$ \tnote{``$C_Q$ that corresponds to $\cV$'' is confusing. How about we define this precisely and use notation such as $C_Q(1^n,\cV)$?} in time $\poly \log_{|\cV|} (n)$.
\end{lemma}
\begin{proof}
...
\begin{enumerate}
	...
	\item The $\sO$ register is set to $1$ if and only if the $\sK_1$ register is all zero. \tnote{is this the only reason for $\sO$? Given later you consider measuring complete registers, $\sK_2\sK_2'$, why not do the same for $\sK_1$, is $\sO$ used for another reason?}
\end{enumerate}
...
\]
where \tnote{I think it'd be easier to say in words that this is the probability of obtaining a certain outcome, when making a certain measurement on certain registers of the circuit in the picture. You made a nice picture, much easier to parse than the below, which is not really needed}
\begin{enumerate}
