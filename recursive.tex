...
The circuit $UPC_{n,r}$ is meant to be interpreted as the circuit for a simcom game with $\kappa_r$ provers, as described in Section~\ref{sec:prelim_simcom} and depicted in Figure \ref{fig:simcom}. Part of a simcom game are subcircuits $C_Q$, $C_A$, and $P$. In $UPC_{n,r}$, the subcircuits $C_Q$ and $C_A$ are determined by the VTM $V_{UPC}$ that we will describe fully in Section~\ref{sec:vtm_upc}. The subcircuit $P$ is simply a sequence of prover reflection gates $P_1,\ldots,P_{\kappa_r}$. %where $P_i$ acts on $\sE_{1,i}' \sE_{2,i}' \sP_i \sK_{2,i}'$. 
